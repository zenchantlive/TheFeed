...
3. Reinstalled the dependency stack from **PowerShell** (`bun add lucide-react@0.538.0 --save-exact`), pinning to the latest version that still ships the complete icon set.
4. Deleted `.next` to clear the stale Turbopack cache and restarted `bun dev --turbopack`. Compilation completed immediately.
...
3. If it is missing, install a known-good version:
   ```powershell
   bun add lucide-react@0.538.0 --save-exact
