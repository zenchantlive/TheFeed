...
function SmartPrompt({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "inline-flex items-center gap-1.5 sm:gap-2 rounded-full border border-border/40 bg-card/80 px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-medium text-foreground transition-all duration-200",
        "hover:border-primary/40 hover:bg-primary/5 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 touch-manipulation",
        disabled && "opacity-50 cursor-not-allowed",
        className
      )}
    >
      <span className={cn("text-xs sm:text-sm flex-shrink-0", accents[category])}>{icon}</span>
      <span className="truncate text-[0.7rem] sm:text-xs">{text}</span>
    </button>
  );
}
...
export function EnhancedSmartPrompts({ 
  ...
  return (
    <div
      className={cn(
        "animate-in fade-in duration-300 rounded-xl sm:rounded-2xl border border-border/40 bg-card/90 p-3 sm:p-4 shadow-[0_12px_30px_rgba(6,8,20,0.12)]",
        className
      )}
    >
      <div className="mb-2 flex items-center gap-2 text-[0.6rem] sm:text-[0.65rem] font-semibold uppercase tracking-[0.3em] sm:tracking-[0.35em] text-muted-foreground">
      ...
      <div className="mt-2 sm:mt-3 flex flex-wrap items-center justify-between gap-2 text-[0.55rem] sm:text-[0.6rem] uppercase tracking-[0.2em] sm:tracking-[0.25em] text-muted-foreground">
