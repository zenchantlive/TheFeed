...
import { ResourceBottomSheet } from "@/components/map/ResourceBottomSheet";
...
export type EnrichedFoodBank = Omit<NormalizedResourceWithMeta, "phone" | "website" | "description"> & {
  phone: string | null;
  website: string | null;
...
function MapPageView({ foodBanks, services, isAdmin }: MapPageClientProps) {
  ...
  const [, setSelectedPostId] = useState<string | null>(initialPostId);
  ...
  return (
    <div className="flex flex-col lg:flex-row relative overflow-hidden h-full w-full">

      {/* --- MOBILE LAYOUT --- */}

      {/* Mobile Top Header (Overlay) */}
      <div className="absolute top-0 left-0 right-0 z-10 flex flex-col gap-2 bg-background/80 p-2 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-background/60 lg:hidden pointer-events-auto">
        {/* Compact Search Bar */}
        <MapSearchBar
      ...
      {/* Desktop Sidebar */}
      <div className="hidden w-96 flex-col gap-4 border-r border-border/60 bg-background lg:flex lg:overflow-y-auto lg:p-4 z-20">
      ...
      {/* Maps Area (Full screen on mobile, Flex on Desktop) */}
      <div className="relative h-full w-full lg:flex-1">
        ...
        <ResourceBottomSheet
          resources={filteredFoodBanks}
          selectedResource={selectedFoodBank}
          onSelectResource={handleSelectFoodBank}
          isAdmin={isAdmin}
        />
      </div>
    </div>
